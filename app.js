...
function add_material(name, compounds_list, mass_list, default_density, default_thickness) {
    if (compounds_list == undefined) { compounds_list = []; };
    if (mass_list == undefined) { mass_list = []; };
    if (default_density == undefined) { default_density=1;  };
    if (default_thickness == undefined) { default_thickness=1; };
...
function update_materials_in_tables() {
    ...
    $("#materials .panel-title a").each(function() {
        ...
        $("#multilayer button").each(function() {
          if ($.trim($(this).text())=='placeholder') { $(this).html($(this).html().replace("placeholder", material_name)) };
        });
...
function get_materials() {
    ...
    $("#materials .panel").each(function() {
      ...
      $(this).find("tbody input").each(function() {
        if (i % 2 == 0) { material.compounds.push($(this).val()); }
        else { material.mass.push($(this).val()); }
        i++;
      });
